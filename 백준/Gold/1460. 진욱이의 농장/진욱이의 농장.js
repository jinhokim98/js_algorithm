...
const prefixSumBySeed = Array.from({length: 8}, () => Array.from({length: n + 1}, () => Array.from({length: n + 1}).fill(0)));

// 씨앗 별 누적합 구하기
for (let seed = 0; seed < 8; seed++) {
    for (let i = 1; i <= n; i++) {
        for (let j = 1; j <= n; j++) {
            prefixSumBySeed[seed][i][j] = prefixSumBySeed[seed][i][j - 1] + prefixSumBySeed[seed][i - 1][j] - prefixSumBySeed[seed][i - 1][j - 1] + (farms[i][j] === seed ? 1 : 0);
        }
    }
}
